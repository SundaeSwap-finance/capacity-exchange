...
To set up for a new environment you need to:
1. pick your env,
2. fund a wallet with NIGHT and register it for DUST generation (this is used for deploying contracts and also as the CES hot wallet),
3. deploy the counter and token-mint contracts, and
4. set your env vars
...
- Create your wallet mnemonic file at `apps/e2e-tests/wallet-mnemonic.<env>.txt`
...
Deploy the counter and token-mint contracts using the same wallet. Run from `apps/demo/contracts/` (the deploy CLIs resolve ZK config paths relative to cwd):
