...
const push = (value: unknown[], alignment?: unknown) => ({
  push: { storage: false, value: { tag: 'cell', content: alignment === undefined ? { value } : { value, alignment } } },
});
...
describe('a disclosure cell is read by what its alignment declares', () => {
  ...
  // The alignment is what says this is a 32-byte field. Without it, any write of any width
  // aimed at a disclosure slot would be padded and accepted as the secret.
  it('ignores a write that declares no alignment at all', () => {
    expect(slots(program(new Uint8Array(16).fill(9)))).toEqual([]);
  });
