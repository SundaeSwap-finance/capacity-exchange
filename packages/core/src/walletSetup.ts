...
import { DUST_PARAMS, COST_PARAMS } from './params';
...
export interface CreateAndSyncWalletOptions {
  seedHex: string;
...
export async function createAndSyncWallet(options: CreateAndSyncWalletOptions): Promise<WalletConnection> {
  const { seedHex, walletConfig, savedShieldedState, savedDustState, syncTimeoutMs } = options;
