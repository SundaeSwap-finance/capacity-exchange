...
export function validate(networkId: string): void {
  ...
  const priceCurrencies = new Set(
    (priceConfig.priceFormulas ?? []).map((pf: { currency: string }) =>
      pf.currency
    ),
  );
  if (!priceCurrencies.has(contracts.tokenMint.derivedTokenColor)) {
    console.error(
      `Token color ${contracts.tokenMint.derivedTokenColor} not in priceFormulas`,
    );
  ...
  const sponsoredEntry = (priceConfig.sponsoredContracts ?? []).find(
    (sc: { contractAddress: string }) =>
      sc.contractAddress === contracts.counter.contractAddress,
  );
  if (!sponsoredEntry) {
    console.error(
      `Counter contract ${contracts.counter.contractAddress} not in sponsoredContracts`,
    );
    console.error(`Regenerate: NETWORK_ID=${networkId} task deploy`);
    process.exit(1);
  }
  const circuitNames: string[] = sponsoredEntry.circuits?.type === "subset"
    ? sponsoredEntry.circuits.circuitNames ?? []
    : [];
  if (!circuitNames.includes("increment")) {
    console.error(
      `Circuit "increment" not sponsored for counter contract ${contracts.counter.contractAddress}`,
    );
    console.error(`Regenerate: NETWORK_ID=${networkId} task deploy`);
    process.exit(1);
  }
